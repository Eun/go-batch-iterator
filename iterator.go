...
// NextBatchFunction is the function that will be called by the iterator to fetch the next batch.
// It returns the items for the batch and indicates whether there are more items available or not.
// If it is unclear whether there are more items or not, it should return true, the iterator will call the function
// again. In this case the execution can be delayed by using the rate limit option (WithRateLimit).
...
// the two cases.
//
// It calls the NextBatchFunc after it all items in the batch have been depleted.
...
// Value returns the current value in the batch.
